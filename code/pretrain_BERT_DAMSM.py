"""
train image_encoder for BERT text_encoder
"""
...
from datasets import prepare_data, PRETRAINED_WEIGHTS

from model import CNN_ENCODER, BERT_ENCODER
...
def train(dataloader, cnn_model, rnn_model, batch_size,
    ...
    s_total_loss1 = torch.tensor(0.)
    w_total_loss0 = torch.tensor(0.)
    w_total_loss1 = torch.tensor(0.)
    ...
    for step, data in enumerate(dataloader, 0):
        ...
        if step % UPDATE_INTERVAL == 0:
            ...
            # attention Maps
            img_set, _ = \
                build_super_images(imgs[-1].cpu(), captions, dataloader.dataset.tokenizer,
                                   attn_maps, att_sze)
...
def evaluate(dataloader, cnn_model, rnn_model, batch_size):
    cnn_model.eval()
    rnn_model.eval()
    s_total_loss = torch.tensor(0.)
    w_total_loss = torch.tensor(0.)
    step = -1
    for step, data in enumerate(dataloader, 0):
...
if __name__ == "__main__":
    ...
    output_dir = '../output/%s_%s_fine-tune%d_%s' % \
        (cfg.DATASET_NAME, cfg.CONFIG_NAME, args.start_fine_tuning, timestamp)
