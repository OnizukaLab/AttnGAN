...
def drawCaption(convas, captions, ixtoword, vis_size, off1=2, off2=2):
    num = captions.size(0)
    img_txt = Image.fromarray(np.squeeze(convas))
    # get a font
...
def build_super_images(real_imgs, captions, ixtoword,
    ...
    middle_pad = np.zeros([pad_sze[2], 2, NCH])
    ...
    for i in range(num):
        ...
        attn = attn.repeat(1, 3, 1, 1).data.numpy() if NCH == 3 else attn.data.numpy()
        ...
        for j in range(seq_len + 1):
            if j < num_attn:
                ...
                one_map *= 255
                #
                PIL_im = Image.fromarray(np.uint8(np.squeeze(img)))
        ...
        row = np.squeeze(np.concatenate(row, 1))
        row_merge = np.squeeze(np.concatenate(row_merge, 1))
